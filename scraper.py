#!/usr/bin/env python3
"""
Booli.se Async Scraper Module - v2

Two-phase scraper:
1. Collects all listing URLs from search pages
...
class AsyncBooliScraper:
    ...
    async def extract_listing_urls(self, page: Page) -> list[str]:
        ...
        try:
            await page.wait_for_selector('a[href*="/bostad/"], a[href*="/annons/"]', timeout=15000)
        except Exception:
            return urls
        
        elements = await page.query_selector_all('a[href*="/bostad/"], a[href*="/annons/"]')
        
        for elem in elements:
            try:
                href = await elem.get_attribute('href') or ""
                if '/bostad/' in href or '/annons/' in href:
                    if href.startswith('/'):
    ...
    async def extract_listing_details(self, page: Page, url: str) -> Optional[dict]:
    ...
    async def url_collector_worker(self, context: BrowserContext, page_queue: asyncio.Queue):
        ...
        while True:
            try:
                page_num = await asyncio.wait_for(page_queue.get(), timeout=5.0)
    ...
    async def detail_scraper_worker(self, context: BrowserContext, url_queue: asyncio.Queue):
        ...
        while True:
            try:
                url = await asyncio.wait_for(url_queue.get(), timeout=10.0)
            except asyncio.TimeoutError:
                break
            
            details = await self.extract_listing_details(page, url)
    ...
    async def scrape(self, max_pages: Optional[int] = None, max_listings: Optional[int] = None) -> list[dict]:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                print(f"\n=== Phase 1: Collecting URLs from {self.total_pages} pages ===")
                
                first_urls = await self.extract_listing_urls(page)
